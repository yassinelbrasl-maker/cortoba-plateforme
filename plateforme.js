...
function genNasPath(code, clientObj){
  ...
  return '\\\\NAS\\Projets\\'+year+'\\'+code+(suffix?'_'+suffix:'');
}
...
function saveProjet(){
  ...
  var body = {
    ...
    typeBat: typeBat||null,
    type_bat: typeBat||null,
...
// Helper : auto-calc coût/m² dans modal-projet édition
function pjAutoCalcCoutM2(){
  var shon=parseFloat((document.getElementById('pj-shon')||{}).value)||0;
  var cout=parseFloat((document.getElementById('pj-cout-construction')||{}).value)||0;
  var m2El=document.getElementById('pj-cout-m2');
  if(m2El&&shon>0&&cout>0) m2El.value=Math.round(cout/shon);
}

// ═══════════════════════════════════════════════════════════════
//  CONFIGURATEUR DE PROJET — Wizard plein écran 6 étapes
// ═══════════════════════════════════════════════════════════════
var _cfgStep=1, _cfgMap=null, _cfgMarker=null, _cfgM2Auto=true, _cfgItvCount=0;
var CFG_TOTAL=6;
var CFG_META=[{n:1,label:'Identité'},{n:2,label:'Surfaces'},{n:3,label:'Coûts'},{n:4,label:'Missions'},{n:5,label:'Localisation'},{n:6,label:'Client'}];
var CFG_BENCH={'Économique':{min:700,max:1100},'Standard':{min:1100,max:1700},'Moyen-Haut':{min:1700,max:2600},'Haut standing':{min:2600,max:4200},'Luxe':{min:4200,max:7500}};
var CFG_ZONE_COEFF={'Urbaine':1.00,'Périurbaine':0.92,'Rurale':0.82,'Agricole':0.75};
...
function closeConfigurateur(){
  var ov=document.getElementById('configurateur-overlay'); if(!ov) return;
  if(((document.getElementById('cfg-nom')||{}).value||'').trim()) cfgSaveDraft();
  ov.style.display='none';
  document.body.style.overflow='';
}

function cfgReset(){
  _cfgStep=1; _cfgItvCount=0; _cfgM2Auto=true;
  ['cfg-nom','cfg-annee','cfg-delai','cfg-description','cfg-shon','cfg-shob','cfg-terrain','cfg-cout-construction','cfg-cout-m2','cfg-honoraires','cfg-budget','cfg-adresse','cfg-lat','cfg-lng'].forEach(function(id){
    var el=document.getElementById(id); if(el) el.value=id==='cfg-annee'?new Date().getFullYear():'';
  });
  ['cfg-type-bat','cfg-phase','cfg-statut'].forEach(function(id){ var el=document.getElementById(id); if(el) el.selectedIndex=0; });
  var e1=document.getElementById('cfg-err-1'); if(e1) e1.style.display='none';
  var e6=document.getElementById('cfg-err-6'); if(e6) e6.style.display='none';
  var il=document.getElementById('cfg-intervenants-list'); if(il) il.innerHTML='';
  var cd=document.getElementById('cfg-coords-display'); if(cd) cd.style.display='none';
  var ce=document.getElementById('cfg-cl-existant'); if(ce) ce.checked=true;
  setTimeout(cfgToggleClientMode,10);
  var ph=document.getElementById('cfg-cl-type-ph'); if(ph) ph.checked=true;
  setTimeout(cfgTogglePersonneType,10);
  for(var i=1;i<=CFG_TOTAL;i++){ var s=document.getElementById('cfg-s'+i); if(s) s.style.display=i===1?'block':'none'; }
  cfgUpdateFooter();
}

function cfgGoStep(n){
  if(n>_cfgStep&&!cfgValidate(_cfgStep)) return;
  var cur=document.getElementById('cfg-s'+_cfgStep), nxt=document.getElementById('cfg-s'+n);
  if(cur) cur.style.display='none'; if(nxt) nxt.style.display='block';
  _cfgStep=n; cfgRenderProgress(); cfgUpdateFooter();
  var ov=document.getElementById('configurateur-overlay'); if(ov) ov.scrollTop=0;
  if(n===3) cfgUpdateBenchmark();
  if(n===4) cfgPopulateCfgMissions();
  if(n===5) setTimeout(cfgInitMap,200);
  if(n===6) cfgPopulateCfgClientSelect();
  cfgSaveDraft();
  setTimeout(function(){ var s=document.getElementById('cfg-s'+n); if(!s) return; var inp=s.querySelector('input:not([type=hidden]):not([type=radio]):not([type=checkbox]),select,textarea'); if(inp) inp.focus(); },160);
}
function cfgNext(){ cfgGoStep(_cfgStep+1); }
function cfgPrev(){ cfgGoStep(_cfgStep-1); }

function cfgValidate(n){
  if(n===1){
    var nom=(document.getElementById('cfg-nom').value||'').trim();
    if(!nom){ cfgShowErr(1,'Le nom du projet est obligatoire.'); document.getElementById('cfg-nom').focus(); return false; }
    cfgHideErr(1);
  }
  if(n===6){
    var mode=document.querySelector('input[name="cfg-cl-mode"]:checked'); if(!mode) return false;
    if(mode.value==='existant'){
      var sel=document.getElementById('cfg-cl-select');
      if(!sel||!sel.value){ cfgShowErr(6,'Veuillez sélectionner un client.'); return false; }
    } else {
      var type=document.querySelector('input[name="cfg-cl-type"]:checked');
      if(!type||type.value==='physique'){ if(!(document.getElementById('cfg-cl-nom').value||'').trim()){ cfgShowErr(6,'Le nom du client est obligatoire.'); return false; } }
      else { if(!(document.getElementById('cfg-cl-raison').value||'').trim()){ cfgShowErr(6,'La raison sociale est obligatoire.'); return false; } }
    }
    cfgHideErr(6);
  }
  return true;
}
function cfgShowErr(s,m){ var el=document.getElementById('cfg-err-'+s); if(el){ el.textContent=m; el.style.display='block'; } }
function cfgHideErr(s){ var el=document.getElementById('cfg-err-'+s); if(el) el.style.display='none'; }
function cfgClearErr(s){ cfgHideErr(s); }

function cfgRenderProgress(){
  var bar=document.getElementById('cfg-progress-bar');
  if(bar){
    bar.innerHTML='';
    for(var i=1;i<=CFG_TOTAL;i++){
      var seg=document.createElement('div'); var done=i<_cfgStep;
      seg.style.cssText='flex:1;height:3px;border-radius:2px;background:'+(i<=_cfgStep?'var(--accent)':'var(--border)')+';transition:background .3s';
      if(done){ (function(st){ seg.style.cursor='pointer'; seg.addEventListener('click',function(){ cfgGoStep(st); }); })(i); }
      bar.appendChild(seg);
    }
  }
  var ctr=document.getElementById('cfg-step-counter'); if(ctr) ctr.textContent='Étape '+_cfgStep+' / '+CFG_TOTAL;
  var tabs=document.getElementById('cfg-step-tabs');
  if(tabs) tabs.innerHTML=CFG_META.map(function(s){
    var active=s.n===_cfgStep, done=s.n<_cfgStep;
    return '<button onclick="'+(done?'cfgGoStep('+s.n+')':'')+'" style="background:none;border:none;border-bottom:2px solid '+(active?'var(--accent)':'transparent')+';padding:0.45rem 0.8rem;font-size:0.72rem;color:'+(active?'var(--accent)':done?'var(--text-2)':'var(--text-3)')+';cursor:'+(done?'pointer':'default')+';white-space:nowrap;letter-spacing:0.06em;transition:color .2s">'+(done?'✓ ':'')+s.label+'</button>';
  }).join('');
}
function cfgUpdateFooter(){
  var prev=document.getElementById('cfg-prev-btn'), next=document.getElementById('cfg-next-btn'), save=document.getElementById('cfg-save-btn');
  if(prev) prev.style.visibility=_cfgStep>1?'visible':'hidden';
  if(next) next.style.display=_cfgStep<CFG_TOTAL?'inline-flex':'none';
  if(save) save.style.display=_cfgStep===CFG_TOTAL?'inline-flex':'none';
}

function cfgRenderOptions(containerId, values, cls){
  var c=document.getElementById(containerId); if(!c) return;
  c.innerHTML=values.map(function(v){
    return '<button type="button" class="'+cls+'" data-value="'+v+'" onclick="cfgSelectOpt(this,\''+cls+'\')" style="padding:0.55rem 0.95rem;border:1px solid var(--border);border-radius:6px;background:var(--bg-2);color:var(--text-2);cursor:pointer;font-size:0.8rem;transition:all .2s;white-space:nowrap">'+v+'</button>';
  }).join('');
}
function cfgSelectOpt(btn, cls){
  document.querySelectorAll('.'+cls).forEach(function(b){ b.style.borderColor='var(--border)'; b.style.background='var(--bg-2)'; b.style.color='var(--text-2)'; b.removeAttribute('data-sel'); });
  ...
  if(_cfgStep===3) cfgUpdateBenchmark();
}
function cfgGetSel(cls){ var el=document.querySelector('.'+cls+'[data-sel="1"]'); return el?el.getAttribute('data-value'):null; }

function cfgAutoCalcCout(){
  if(!_cfgM2Auto) return;
  var shon=parseFloat((document.getElementById('cfg-shon')||{}).value)||0;
  var cout=parseFloat((document.getElementById('cfg-cout-construction')||{}).value)||0;
  var m2El=document.getElementById('cfg-cout-m2');
  if(m2El&&shon>0&&cout>0) m2El.value=Math.round(cout/shon);
  if(_cfgStep===3) cfgUpdateBenchmark();
}
function cfgToggleCoutM2Auto(){
  _cfgM2Auto=!_cfgM2Auto;
  var btn=document.getElementById('cfg-m2-auto-btn'), m2El=document.getElementById('cfg-cout-m2'), lbl=document.getElementById('cfg-cout-m2-lbl');
  if(btn){ btn.style.opacity=_cfgM2Auto?'1':'0.45'; btn.style.background=_cfgM2Auto?'var(--accent-bg)':'var(--bg-2)'; }
  if(m2El) m2El.readOnly=_cfgM2Auto;
  if(lbl) lbl.textContent=_cfgM2Auto?'calculé automatiquement':'saisie manuelle';
  if(_cfgM2Auto) cfgAutoCalcCout();
}

function cfgUpdateBenchmark(){
  var standing=cfgGetSel('cfg-std-opt'), zone=cfgGetSel('cfg-zone-opt');
  var lbl=document.getElementById('cfg-bench-label'), rng=document.getElementById('cfg-bench-range');
  var tWrap=document.getElementById('cfg-bench-total-wrap'), tEl=document.getElementById('cfg-bench-total'), sVal=document.getElementById('cfg-bench-shon-val');
  var bar=document.getElementById('cfg-bench-bar'), posLbl=document.getElementById('cfg-bench-pos-label');
  if(!standing||!zone){ if(lbl) lbl.textContent='sélectionnez le standing et la zone (étape 2)'; if(rng) rng.textContent='—'; return; }
  var b=CFG_BENCH[standing]; if(!b) return;
  var coeff=CFG_ZONE_COEFF[zone]||1, minA=Math.round(b.min*coeff), maxA=Math.round(b.max*coeff);
  ...
  var shon=parseFloat((document.getElementById('cfg-shon')||{}).value)||0;
  if(shon>0){ if(tWrap) tWrap.style.display='block'; if(sVal) sVal.textContent=shon.toLocaleString('fr-FR'); if(tEl) tEl.textContent=Math.round(minA*shon).toLocaleString('fr-FR')+' — '+Math.round(maxA*shon).toLocaleString('fr-FR')+' TND'; }
  else{ if(tWrap) tWrap.style.display='none'; }
  var cm2=parseFloat((document.getElementById('cfg-cout-m2')||{}).value)||0;
  if(cm2>0&&maxA>0){
...
function cfgPopulateCfgMissions(){
  var list=document.getElementById('cfg-missions-list'); if(!list) return;
  var missions=getMissions();
  if(!missions.length){ list.innerHTML='<span style="font-size:0.82rem;color:var(--text-3)">Aucune mission configurée. Rendez-vous dans Paramètres.</span>'; return; }
  list.innerHTML=missions.map(function(m){
    var val=(m.abbr||m.id)+'_'+(m.nom||'');
    return '<label style="display:flex;align-items:center;gap:0.5rem;padding:0.5rem 0.65rem;background:var(--bg-2);border-radius:6px;cursor:pointer;border:1px solid transparent;transition:border-color .15s" onchange="this.style.borderColor=this.querySelector(\'input\').checked?\'var(--accent)\':\'transparent\'"><input type="checkbox" value="'+val+'" style="accent-color:var(--accent)"><span style="font-size:0.8rem"><strong style="color:var(--accent);margin-right:0.3rem">'+(m.abbr||'')+'</strong>'+(m.nom||m.abbr||'')+'</span></label>';
  }).join('');
}
function cfgGetCfgMissions(){ return Array.from(document.querySelectorAll('#cfg-missions-list input[type=checkbox]:checked')).map(function(c){ return c.value; }); }

function cfgAddIntervenant(){
  _cfgItvCount++;
  var n=_cfgItvCount, list=document.getElementById('cfg-intervenants-list'); if(!list) return;
  var roles=getRoles();
  var opts=roles.length?roles.map(function(r){ return '<option>'+(r.nom||r)+'</option>'; }).join(''):'<option>Bureau structure</option><option>Bureau fluides</option><option>Géomètre</option><option>Économiste</option><option>Autre</option>';
  ...
  row.innerHTML='<select class="form-select" style="width:155px;flex-shrink:0"><option value="">Rôle</option>'+opts+'</select><input class="form-input" placeholder="Nom / Cabinet" style="flex:1"/><input class="form-input" placeholder="Contact" style="flex:1"/><button type="button" onclick="document.getElementById(\'cfg-itv-'+n+'\').remove()" style="background:none;border:none;color:var(--text-3);cursor:pointer;padding:0.1rem 0.4rem;font-size:1rem;flex-shrink:0" title="Supprimer">✕</button>';
  list.appendChild(row);
}
function cfgGetIntervenants(){ return Array.from(document.querySelectorAll('[id^="cfg-itv-"]')).map(function(r){ var sel=r.querySelector('select'); var inp=r.querySelectorAll('input'); return {role:sel?sel.value:'',nom:inp[0]?inp[0].value:'',contact:inp[1]?inp[1].value:''}; }).filter(function(i){ return i.nom.trim(); }); }

function cfgInitMap(){
  var c=document.getElementById('cfg-map-container'); if(!c||!window.L) return;
  if(_cfgMap){ _cfgMap.invalidateSize(); return; }
  _cfgMap=L.map('cfg-map-container').setView([33.8869,9.5375],7);
  L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',{attribution:'© OpenStreetMap',maxZoom:19}).addTo(_cfgMap);
  _cfgMap.on('click',function(e){
    var lat=e.latlng.lat, lng=e.latlng.lng;
    if(_cfgMarker) _cfgMarker.setLatLng([lat,lng]); else _cfgMarker=L.marker([lat,lng]).addTo(_cfgMap);
    document.getElementById('cfg-lat').value=lat; document.getElementById('cfg-lng').value=lng;
    document.getElementById('cfg-lat-display').textContent=lat.toFixed(6); document.getElementById('cfg-lng-display').textContent=lng.toFixed(6);
    var link=document.getElementById('cfg-maps-link'); if(link) link.href='https://maps.google.com/?q='+lat+','+lng;
    document.getElementById('cfg-coords-display').style.display='block';
  });
}

function cfgPopulateCfgClientSelect(){
  var sel=document.getElementById('cfg-cl-select'); if(!sel) return;
  var clients=getClients();
  sel.innerHTML='<option value="">— Sélectionner un client —</option>'+clients.map(function(c){ var nom=c.displayNom||c.display_nom||c.nom||c.raison||''; return '<option value="'+c.id+'">'+nom+' ('+(c.code||'')+')</option>'; }).join('');
}
function cfgToggleClientMode(){
  var mode=document.querySelector('input[name="cfg-cl-mode"]:checked');
  var pEx=document.getElementById('cfg-cl-panel-ex'), pNv=document.getElementById('cfg-cl-panel-nv');
  var lEx=document.getElementById('cfg-cl-lbl-ex'), lNv=document.getElementById('cfg-cl-lbl-nv');
  var isEx=!mode||mode.value==='existant';
  if(pEx) pEx.style.display=isEx?'block':'none'; if(pNv) pNv.style.display=isEx?'none':'block';
  if(lEx){ lEx.style.borderColor=isEx?'var(--accent)':'var(--border)'; lEx.style.background=isEx?'var(--accent-bg)':''; }
  if(lNv){ lNv.style.borderColor=isEx?'var(--border)':'var(--accent)'; lNv.style.background=isEx?'':'var(--accent-bg)'; }
}
function cfgTogglePersonneType(){
  var type=document.querySelector('input[name="cfg-cl-type"]:checked');
  var fph=document.getElementById('cfg-cl-f-ph'), fmo=document.getElementById('cfg-cl-f-mo');
  var isph=!type||type.value==='physique';
  if(fph) fph.style.display=isph?'grid':'none'; if(fmo) fmo.style.display=isph?'none':'grid';
  var lph=document.getElementById('cfg-lbl-physique'), lmo=document.getElementById('cfg-lbl-morale');
  if(lph) lph.style.borderColor=isph?'var(--accent)':'var(--border)';
  if(lmo) lmo.style.borderColor=isph?'var(--border)':'var(--accent)';
}
function cfgShowClientPreview(){
  var sel=document.getElementById('cfg-cl-select'), preview=document.getElementById('cfg-cl-preview'), content=document.getElementById('cfg-cl-preview-content');
  if(!sel||!sel.value){ if(preview) preview.style.display='none'; return; }
  var client=getClients().find(function(c){ return c.id===sel.value; }); if(!client||!preview||!content) return;
  var nom=client.displayNom||client.display_nom||client.nom||'';
  var rows=[['Client','<strong>'+nom+'</strong>'],client.email?['Email',client.email]:null,client.tel?['Tél',client.tel]:null,client.adresse?['Adresse',client.adresse]:null,['Statut','<span class="'+badgeClass(client.statut||'')+'">'+( client.statut||'—')+'</span>'],client.source?['Source',client.source]:null].filter(Boolean);
  content.innerHTML='<table style="width:100%;border-collapse:collapse">'+rows.map(function(r){ return '<tr><td style="padding:0.3rem 1rem 0.3rem 0;color:var(--text-3);font-size:0.75rem;white-space:nowrap;vertical-align:top">'+r[0]+'</td><td style="padding:0.3rem 0;font-size:0.82rem">'+r[1]+'</td></tr>'; }).join('')+'</table>';
  preview.style.display='block';
}

function cfgSaveDraft(){
  try{
    var d={nom:(document.getElementById('cfg-nom')||{}).value,typeBat:(document.getElementById('cfg-type-bat')||{}).value,phase:(document.getElementById('cfg-phase')||{}).value,statut:(document.getElementById('cfg-statut')||{}).value,annee:(document.getElementById('cfg-annee')||{}).value,delai:(document.getElementById('cfg-delai')||{}).value,description:(document.getElementById('cfg-description')||{}).value,shon:(document.getElementById('cfg-shon')||{}).value,shob:(document.getElementById('cfg-shob')||{}).value,terrain:(document.getElementById('cfg-terrain')||{}).value,standing:cfgGetSel('cfg-std-opt'),zone:cfgGetSel('cfg-zone-opt'),coutConst:(document.getElementById('cfg-cout-construction')||{}).value,coutM2:(document.getElementById('cfg-cout-m2')||{}).value,honoraires:(document.getElementById('cfg-honoraires')||{}).value,budget:(document.getElementById('cfg-budget')||{}).value,adresse:(document.getElementById('cfg-adresse')||{}).value,step:_cfgStep,ts:Date.now()};
...
function cfgRestoreDraft(){
  try{
    ...
    var hint=document.getElementById('cfg-hint');
    if(hint) hint.innerHTML='<span style="color:var(--accent)">Brouillon : '+d.nom+'</span> — <button onclick="cfgApplyDraft()" style="background:none;border:none;color:var(--accent);cursor:pointer;text-decoration:underline;font-size:0.7rem;padding:0">Restaurer</button> · <button onclick="cfgDiscardDraft()" style="background:none;border:none;color:var(--text-3);cursor:pointer;text-decoration:underline;font-size:0.7rem;padding:0">Ignorer</button>';
  }catch(e){}
}
function cfgApplyDraft(){
  try{
    ...
    sv('cfg-nom',d.nom);sv('cfg-type-bat',d.typeBat);sv('cfg-phase',d.phase);sv('cfg-statut',d.statut);sv('cfg-annee',d.annee);sv('cfg-delai',d.delai);sv('cfg-description',d.description);sv('cfg-shon',d.shon);sv('cfg-shob',d.shob);sv('cfg-terrain',d.terrain);sv('cfg-cout-construction',d.coutConst);sv('cfg-cout-m2',d.coutM2);sv('cfg-honoraires',d.honoraires);sv('cfg-budget',d.budget);sv('cfg-adresse',d.adresse);
    if(d.standing){ var sb=document.querySelector('.cfg-std-opt[data-value="'+d.standing+'"]'); if(sb) cfgSelectOpt(sb,'cfg-std-opt'); }
    if(d.zone){ var zb=document.querySelector('.cfg-zone-opt[data-value="'+d.zone+'"]'); if(zb) cfgSelectOpt(zb,'cfg-zone-opt'); }
...
function cfgDiscardDraft(){ localStorage.removeItem('cortoba_cfg_draft'); var hint=document.getElementById('cfg-hint'); if(hint) hint.textContent=''; }

function cfgSave(){
  if(!cfgValidate(6)) return;
  var mode=document.querySelector('input[name="cfg-cl-mode"]:checked'); if(!mode) return;
  var saveBtn=document.getElementById('cfg-save-btn');
  if(saveBtn){ saveBtn.disabled=true; saveBtn.textContent='Création…'; }
  function doCreateProjet(clientId, clientNom, clientCode){
    var annee=parseInt((document.getElementById('cfg-annee')||{}).value)||new Date().getFullYear();
    var code=genProjetCode(annee,clientNom,clientCode);
    var nasPath=genNasPath(code,{displayNom:clientNom,code:clientCode});
    var body={
      nom:(document.getElementById('cfg-nom').value||'').trim(),
      client:clientNom,clientId:clientId,clientCode:clientCode,code:code,annee:annee,
      phase:document.getElementById('cfg-phase').value,
      statut:document.getElementById('cfg-statut').value,
      typeBat:document.getElementById('cfg-type-bat').value||null,
      type_bat:document.getElementById('cfg-type-bat').value||null,
      delai:document.getElementById('cfg-delai').value||null,
      honoraires:parseFloat(document.getElementById('cfg-honoraires').value)||0,
      budget:parseFloat(document.getElementById('cfg-budget').value)||null,
      surface:parseFloat(document.getElementById('cfg-shon').value)||null,
      surface_shon:parseFloat(document.getElementById('cfg-shon').value)||null,
      surface_shob:parseFloat(document.getElementById('cfg-shob').value)||null,
      surface_terrain:parseFloat(document.getElementById('cfg-terrain').value)||null,
      standing:cfgGetSel('cfg-std-opt')||null,
      zone:cfgGetSel('cfg-zone-opt')||null,
      cout_construction:parseFloat(document.getElementById('cfg-cout-construction').value)||null,
      cout_m2:parseFloat(document.getElementById('cfg-cout-m2').value)||null,
      description:(document.getElementById('cfg-description').value||'').trim()||null,
      adresse:(document.getElementById('cfg-adresse').value||'').trim()||null,
      lat:parseFloat(document.getElementById('cfg-lat').value)||null,
      lng:parseFloat(document.getElementById('cfg-lng').value)||null,
      nasPath:nasPath,nas_path:nasPath,
      missions:cfgGetCfgMissions(),intervenants:cfgGetIntervenants()
    };
    apiFetch('api/projets.php',{method:'POST',body:body})
      .then(function(){
        localStorage.removeItem('cortoba_cfg_draft');
        loadData().then(function(){ renderProjets(); populateProjetSelect(); });
        closeConfigurateur(); showPage('projets');
      })
      .catch(function(e){ cfgShowErr(6,'Erreur : '+(e.message||'Erreur serveur')); if(saveBtn){ saveBtn.disabled=false; saveBtn.textContent='Créer le projet →'; } });
  }
  if(mode.value==='existant'){
    var sel=document.getElementById('cfg-cl-select');
    var client=getClients().find(function(c){ return c.id===sel.value; })||{};
    var nom=client.displayNom||client.display_nom||client.nom||client.raison||'';
    doCreateProjet(client.id,nom,client.code||'');
  } else {
    var clType=document.querySelector('input[name="cfg-cl-type"]:checked');
    var isPh=!clType||clType.value==='physique';
    var prenom=(document.getElementById('cfg-cl-prenom').value||'').trim();
    var cnom=(document.getElementById('cfg-cl-nom').value||'').trim();
    var raison=(document.getElementById('cfg-cl-raison').value||'').trim();
    var matricule=(document.getElementById('cfg-cl-matricule').value||'').trim();
    var displayNom=isPh?(prenom?(prenom+' '+cnom).trim():cnom):raison;
    var clBody={type:isPh?'physique':'morale',prenom:prenom||null,nom:cnom||null,raison:raison||null,matricule:matricule||null,display_nom:displayNom,email:(document.getElementById('cfg-cl-email').value||'').trim()||null,tel:(document.getElementById('cfg-cl-tel').value||'').trim()||null,adresse:(document.getElementById('cfg-cl-adresse-cl').value||'').trim()||null,statut:document.getElementById('cfg-cl-statut').value||'Actif',source:document.getElementById('cfg-cl-source').value||null};
    apiFetch('api/clients.php',{method:'POST',body:clBody})
      .then(function(resp){ var nc=resp.data||resp; return loadData().then(function(){ doCreateProjet(nc.id,displayNom,nc.code||''); }); })
      .catch(function(e){ cfgShowErr(6,'Erreur création client : '+(e.message||'Erreur')); if(saveBtn){ saveBtn.disabled=false; saveBtn.textContent='Créer le projet →'; } });
  }
}
...
// Correspondance page → bouton sidebar
var NAV_MODULE_MAP = {
  'dashboard':   '[onclick="showPage('dashboard')"]',
  'devis':       '[onclick="showPage('devis')"]',
  'projets':     '[onclick="showPage('projets')"]',
  'facturation': '[onclick="showPage('facturation')"]',
  'bilans':      '[onclick="showPage('bilans')"]',
  'depenses':    '[onclick="showPage('depenses')"]',
  'fiscalite':   '[onclick="showPage('fiscalite')"]',
  'nas':         '[onclick="showPage('nas')"]',
  'equipe':      '[onclick="showPage('equipe')"]',
  'clients':     '[onclick="showPage('clients')"]',
  'parametres':  '[onclick="showPage('parametres')"]',
};
...
function applyModuleAccess() {
  ...
  Object.keys(NAV_MODULE_MAP).forEach(function(moduleId) {
    var selector = NAV_MODULE_MAP[moduleId];
    ...
    if (hasAccess) {
      btn.style.display = '';
      btn.parentElement && btn.parentElement.style && (btn.parentElement.style.display = '');
    } else {
      btn.style.display = 'none';
    }
  });

  // Masquer les sidebar-section vides (toutes leurs entrées cachées)
  document.querySelectorAll('.sidebar-section').forEach(function(section) {
    var visibleItems = Array.from(section.querySelectorAll('.nav-item')).filter(function(b) {
      return b.style.display !== 'none';
    });
    section.style.display = visibleItems.length === 0 ? 'none' : '';
  });

  // Afficher/masquer les sections admin dans Paramètres
  var s = getSession();
  var isAdmin = s && s.isAdmin;
  var adminOnlySections = ['param-roles-card'];
...
function showPage(id){
  ...
  if(id==='parametres') {
    ...
    setTimeout(function(){
      ...
      loadAgenceParams();
      // NAS
      var nl = document.getElementById('param-nas-local');
      if(nl) nl.value = getSetting('cortoba_nas_local', '192.168.1.100:5000');
      var nc = document.getElementById('param-nas-cloud');
      if(nc) nc.value = getSetting('cortoba_nas_cloud', getNasUrl());
      // Logo
      loadLogoParam();
...
// ── NAS ──
function nasConnect(){
  var btn=document.getElementById('nas-connect-btn'); btn.textContent='Connexion…';
  setTimeout(function(){
    btn.textContent='✓ Connecté'; btn.style.background='var(--green)'; btn.style.borderColor='var(--green)';
    setTimeout(function(){
      btn.innerHTML='<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M18 13v6a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h6"/><polyline points="15 3 21 3 21 9"/><line x1="10" y1="14" x2="21" y2="3"/></svg> Accéder au NAS';
      btn.style.background='var(--accent)'; btn.style.borderColor='var(--accent)';
      var nasAddr = getSetting('cortoba_nas_local','192.168.1.100:5000');
      var nasUrl = nasAddr.indexOf('http')===0 ? nasAddr : 'http://'+nasAddr;
