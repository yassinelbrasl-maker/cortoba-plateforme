...
function applyModuleAccess() {
  ...
  NAV_MODULE_IDS.forEach(function(moduleId) {
    // Trouver le bouton sidebar par sa classe nav-item et son contenu onclick
    var allNavBtns = document.querySelectorAll('.nav-item');
    var btn = null;
    for (var i = 0; i < allNavBtns.length; i++) {
      var onclick = allNavBtns[i].getAttribute('onclick') || '';
      if (onclick.indexOf("showPage('" + moduleId + "')") !== -1) {
        btn = allNavBtns[i]; break;
      }
    }
    if (!btn) return;
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  if (token) {
    apiFetch('api/auth.php?action=me')
      .then(function(r){
        var u = r.data || r;
        // Mettre à jour la session avec les modules retournés par /me
        var existing = JSON.parse(sessionStorage.getItem('cortoba_session') || '{}');
        existing.name    = u.name || existing.name;
        existing.modules = u.modules || existing.modules || null;
        existing.isAdmin = existing.isAdmin || (u.role === 'admin');
        sessionStorage.setItem('cortoba_session', JSON.stringify(existing));
        var nameEl = document.getElementById('user-display');
